...
export default function ClientPage({ initialProducts, initialProductsData }: { initialProducts?: string, initialProductsData?: any[] }) {
  const [user, setUser] = useState<User | null>(null);
  const [isAdmin, setIsAdmin] = useState(false);
  const [loading, setLoading] = useState(true);
  const [showWelcomePopup, setShowWelcomePopup] = useState(false);

  // Build initial product HTML from data if provided
  useEffect(() => {
    if (initialProductsData && initialProductsData.length > 0) {
      const grid = document.getElementById('main-product-grid');
      if (!grid) return;
      
      const html = initialProductsData.map((p: any, idx: number) => {
  ...
  <section class="section-three">
    <div class="product-grid" id="main-product-grid">
      {/* Products will be injected by useEffect */}
    </div>
  </section>
