...
export default async function HomePage() {
  ...
  try {
    const res = await fetch(
      'https://firestore.googleapis.com/v1/projects/bearhasflower/databases/(default)/documents/products?pageSize=100',
      { cache: 'no-store' }
    );
      ...
      return `
        <article class="product-card fade-in" style="animation-delay: ${0.05 + idx * 0.05}s;" data-product-id="${p.id}" data-product-type="${productType}">
          <div class="product-image-wrap" ${isSoldOut ? '' : `onclick="${productNav}"`} style="cursor:${isSoldOut ? 'default' : 'pointer'}; position:relative; overflow:hidden;">
            ${p.coverImage ? `<img src="${placeholderSrc}" data-real-src="${p.coverImage.startsWith('data:') ? '' : p.coverImage}" alt="${p.name}" class="product-image" loading="${idx < 2 ? 'eager' : 'lazy'}" decoding="async" style="width:100%; height:100%; object-fit:cover; position:absolute; top:0; left:0; border-radius:inherit; background:#f0e6ef;" />` : `<div class="product-placeholder">🌹</div>`}
